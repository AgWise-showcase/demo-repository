...
rundssat <-function(i,path.to.extdata,TRT,AOI=TRUE,crop_code,path_DSSAT){
...
#' @examples dssat.exec(country = "Rwanda",  useCaseName = "RAB", Crop = "Maize", AOI = FALSE, Planting_month_date = NULL,jobs=10,TRT=1:36)
 ...
 dssat.exec <- function(pathOutput,country, useCaseName, Crop, AOI = TRUE,TRT,varietyid, zone, level2=NA,path_DSSAT){  
     
  #Set working directory to save the results
   if(AOI == TRUE){
     path.to.extdata_ini <- paste(pathOutput,"/useCase_", 
  ...
  results <- future_lapply(indices, function(i) {
    ...
    result <- rundssat(i, path.to.extdata=path.to.extdata, TRT=TRT,  AOI=AOI,crop_code=crop_code,path_DSSAT=path_DSSAT)

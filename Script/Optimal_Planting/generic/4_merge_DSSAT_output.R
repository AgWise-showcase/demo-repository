...
merge_DSSAT_output <- function(pathOutput,country, useCaseName, Crop, AOI = FALSE, season = NULL, varietyids, zone_folder = TRUE, level2_folder = FALSE) {
  ...
  if (AOI == TRUE) {
    dir_path <- paste0(pathOutput,"/useCase_", country, "_", useCaseName, "/", Crop, "/result/DSSAT/AOI/")
  } else {
    dir_path <- paste0(pathOutput,"/useCase_", country, "_", useCaseName, "/", Crop, "/result/DSSAT/fieldData/")
  }
